...
last_version=$(curl -Ls "https://api.github.com/repos/qiliy/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/qiliy/3x-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_menu() {
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/qiliy/3x-ui/main/x-ui.sh
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/qiliy/3x-ui/master/install.sh"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/qiliy/3x-ui/master/install.sh)${plain}"
    echo ""
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/qiliy/3x-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
——————————————————————
  ...
  ${yellow}https://github.com/qiliy/3x-ui${plain}
  ${green}详细〔安装配置〕教程${plain}
  ${yellow}https://qiliy.github.io/xufei/2024/05/3x-ui${plain}
——————————————————————
"
    show_status
